...
%>
			   ...
			   try{
				  ...
				  RestaurantReviewDataBean article =  dbPro.getArticle(num);
...
<script type="text/javascript">
	...
	function check01(){
		...
		if(val1==val2){
			document.location.href='deletePro_restaurant_review_admin.jsp?food='+food+'&pageNum='+<%=pageNum%>+
					'&num='+<%=num%>;
			...
			document.location.href='deleteForm_restaurant_review.jsp?food='+food+
		...
		<div id="content-wrapper">
			...
			%>
				...
				<ol class="breadcrumb">
					...
					<li ><a href= "main_restaurant_review2.jsp?jongkak">
					<span style="font-size:20px;">&nbsp;<b>/ 맛집리뷰</b></span></a></li>
					<li ><a href= "restaurant_review.jsp?food=<%=food%>">
				...
				mainnm ="main_restaurant_review2"; %>
				<!-- Breadcrumbs-->
				<ol class="breadcrumb">
					<li ><a href="index.jsp">
					<span style="color: #EF0000; font-size:20px;"><b>ASIANA IDT</b></span></a>
					...
					<table >
						...
						<tr height="30">
							<td colspan="4" bgcolor="#E9ECEF" align="right"><input
								...
								onclick="document.location.href='updateForm_restaurant_review.jsp?food=<%=request.getParameter("food")%>&num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; 
								<input type="button" class="btn btn-secondary" value="글삭제" onclick="check01()"/>
								&nbsp;&nbsp;&nbsp;&nbsp; 
								<input type="hidden" id="user" name="user" value=<%=email %>>
								<input type="hidden" id="food" name="food" value=<%=request.getParameter("food")%>>
								<input type="button" class="btn btn-secondary" value="답글쓰기"
								onclick="document.location.href='writeForm_restaurant_review.jsp?food=<%=request.getParameter("food")%>&num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; 
								<input type="button"class="btn btn-secondary"  value="글목록"
								onclick="document.location.href='restaurant_review.jsp?food=<%=request.getParameter("food")%>'">
