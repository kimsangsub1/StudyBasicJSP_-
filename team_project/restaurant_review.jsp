...
<%
	...
	if (count > 0) {
		articleList = dbPro.getArticles(startRow, pageSize, request.getParameter("food"));
...
<body id="page-top">
   ...
   <!-- Navbar Search -->
	...
	<div id="wrapper">
		...
		<div id="content-wrapper">
			...
			<%
			String mainnm = "";
			if(request.getParameter("food").equals("Baren")||request.getParameter("food").equals("Allback")||request.getParameter("food").equals("Doma")){
			...
			%>
				...
				%> 
					...
					<tr>
						...
						<a href="writeForm_restaurant_review.jsp?food=<%=urlname %>">
						<button type="button" class="btn btn-#E9ECEF" value = "<%=urlname%>">글쓰기</button></a></td>
					</tr>
				</table>
				...
				<table class="table table-hover">
					...
					<%
						for (int i = 0; i < articleList.size(); i++) {
							RestaurantReviewDataBean article = articleList.get(i);
					%>
					<tr height="30">
 ...
 %> <a
							href="content_restaurant_review.jsp?food=<%=request.getParameter("food")%>&num=<%=article.getNum()%>&pageNum=<%=currentPage%>">
 ...
 %>
				...
				<%
					if (count > 0) {
						...
						if (startPage > 10) {
				%>
			<a href="restaurant_review.jsp?food=<%=request.getParameter("food")%>&pageNum=<%=startPage - 10%>"">[이전]</a>
			<%
				}%>
			...
			<table width="100%">
				<tr>
					<td align="center">
						...
						%> <a href="restaurant_review.jsp?food=<%=request.getParameter("food")%>&pageNum=<%=i%>">
						<span style="color: #000000;">[<%=i%>]
						</span> </a> <!-- </div> --> <%
	}

		if (endPage < pageCount) {
%> <a href="restaurent_review.jsp?pageNum=<%=startPage + 10%>">[다음]</a> <%
	}
	}
%>
    ...
    var fired_button2 = $(this).val();
   
    
    location.href="writeForm_restaurant_review.jsp?"+fir
