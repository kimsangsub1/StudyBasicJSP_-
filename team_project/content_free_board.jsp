...
<!-- Page Content -->
			   ...
			   try{
				  ...
				  FreeBoardDataBean article =  dbPro.getArticle(num);
...
<meta name="viewport"
	content="width=device-width, initial-sx`cale=1, shrink-to-fit=no">
...
<script type="text/javascript">
	...
	function check01(){
		...
		if(val1==val2){
			...
			document.location.href='deletePro_free_board_admin.jsp?pageNum='+<%=pageNum%>+
					'&num='+<%=num%>;
					
					alert("관리자권한("+ val1 +") 삭제");
		}else{
			document.location.href='deleteForm_free_board.jsp?num='+<%=article.getNum()%>+'&pageNum='+<%=pageNum%>;
					...
					<table >
						...
						<tr height="30">
							<td colspan="4" bgcolor="#E9ECEF" align="right"><input
								...
								onclick="document.location.href='updateForm_free_board.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; 
								...
								onclick="document.location.href='writeForm_free_board.jsp?num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; <input type="button"class="btn btn-secondary"  value="글목록"
								onclick="document.location.href='free_board.jsp?pageNum=<%=pageNum%>'">
							</td>
						</tr>
					</table>
					<%
