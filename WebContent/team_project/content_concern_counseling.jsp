...
<!-- Page Content -->
				...
				<%
					...
					try {
						...
						ConcernCounselingDataBean article = dbPro.getArticle(num);
...
<script type="text/javascript">
	...
	<%-- alert("val1 : " + val1 +" num : " + <%=num%>); --%>
	
	if(val1==val2){
		
		document.location.href='deletePro_concern_counseling_admin.jsp?pageNum='+<%=pageNum%>+
				'&num='+<%=num%>+'&theme='+theme;
				
				alert("관리자권한("+ val1 +") 삭제");
	}else{
		document.location.href='deleteForm_concern_counseling.jsp?num='+<%=article.getNum()%>+'&pageNum='+<%=pageNum%>
		+'&theme='+theme;
		...
		<div id="content-wrapper">
			...
			<!-- theme있던 부분 -->
				...
				<ol class="breadcrumb">
					...
					<li><a href="concern_counseling.jsp?theme=<%=theme%>"> <span
							style="font-size: 20px;">&nbsp;<b>/ 고민게시판</b></span></a></li>
					<li><a href="#"> <span
							style="font-size: 20px;">&nbsp;<b>/ 글내용 보기</b></span></a></li>
				</ol>
					...
					<table >
						...
						<tr height="30">
							<td colspan="4" bgcolor="#E9ECEF" align="right"><input
								...
								onclick="document.location.href='updateForm_concern_counseling.jsp?theme=<%=theme%>&num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; 
								...
								onclick="document.location.href='writeForm_concern_counseling.jsp?theme=<%=theme%>&num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
								&nbsp;&nbsp;&nbsp;&nbsp; <input type="button"class="btn btn-secondary"  value="글목록"
								onclick="document.location.href='concern_counseling.jsp?theme=<%=theme%>&pageNum=<%=pageNum%>'">
							</td>
						</tr>
					</table>
					<%
